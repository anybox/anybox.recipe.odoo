...
class TestServer(RecipeTestCase):
    ...
    def test_retrieve_addons_vcs(self):
        ...
        self.make_recipe(
            version="10.0", addons="fakevcs http://trunk.example " "addons-trunk rev"
        )
    ...
    def test_retrieve_addons_vcs_order(self):
        ...
        self.make_recipe(
            version="10.0",
            addons=os.linesep.join(
                ["fakevcs http://trunk.example addons-%d rev" % d for d in range(10)]
            ),
        )

        self.recipe.retrieve_addons()
        paths = self.recipe.addons_paths
        expected = [os.path.join(self.buildout_dir, "addons-%d") % d for d in range(10)]
    ...
    def test_retrieve_addons_subdir(self):
        self.make_recipe(
            version="10.0",
            addons="fakevcs lp:odoo-web web " "last:1 subdir=addons bzrinit=branch",
        )
        # manual creation because fakevcs does nothing but retrieve_addons
        # has assertions on existence of target directories
        ...
        self.assertEquals(
            ...
            [
                (
                    ...
                    "last:1",
                    dict(
                        offline=False, clear_locks=False, clean=False, group="grouped"
                    ),
                ),
                (
                    addons2_dir,
    ...
    def test_addons_standalone_oldstyle_prohibited(self):
        ...
        dirname = "standalone"
        self.make_recipe(version="10.0", addons="group custom %s last:1" % dirname)
    ...
    def test_merge_requirements(self):
        ...
        self.assertEquals(
            set(self.recipe.requirements), set(["anybox.recipe.odoo", "odoo"])
        )

    def test_merge_requirements_new_project_name(self):
        """At any point in time, Odoo is prone to change package name."""
        self.make_recipe(
            version="local %s" % os.path.join(TEST_DIR, "odoo-project-renaming")
        )
    ...
    def test_forbid_addons_paths_option(self):
        ...
        self.assertRaises(
            UserError, self.recipe.finalize_addons_paths, check_existence=False
        )

    def install_scripts(self, extra_develop=None, extra_requirements=()):
    ...
    def do_test_install_scripts_soft_deps(self, exc=None):
        ...
        self.recipe.missing_deps_instructions[
            softreq
    ...
    def test_install_scripts_indirect_soft_deps(self, exc=None):
        ...
        self.assertRaises(
            UserError, self.install_scripts, extra_requirements=(somereq,)
        )
    ...
    def test_install_scripts_soft_deps_missing_dist_exc(self):
        ...
        self.do_test_install_scripts_soft_deps(exc=MissingDistribution(req, []))
    ...
    def test_install_scripts_soft_deps_user_error(self):
        self.do_test_install_scripts_soft_deps(
            exc=UserError(
                "We don't have a distribution for "
    ...
    def test_install_scripts_10(self, with_devtools=True, **kw):
        ...
        expected = ["start_odoo", "gunicorn_odoo", "cron_worker_odoo"]
        if with_devtools:
    ...
    def test_gunicorn_preload_databases(self, databases="onedb", expected="('onedb',)"):
        self.make_recipe(
            version="local %s" % os.path.join(TEST_DIR, "odoo10"), gunicorn="direct"
        )
        ...
        gunicorn_conf = os.path.join(self.recipe.etc, "gunicorn_odoo.conf.py")
    ...
    def test_gunicorn_preload_databases_multiple(self):
        self.test_gunicorn_preload_databases(
            databases="db1\ndb2", expected="('db1', 'db2')"
        )
    ...
    def test_install_scripts_10_server_wide_modules(self):
        ...
        self.assertTrue(
            "server_wide_modules=('web', 'anybox_homepage')"
            in self.read_script("start_odoo")
        )
        with open(
            os.path.join(self.buildout_dir, "etc", "gunicorn_odoo.conf.py")
        ) as gu:
            self.assertTrue(
                "odoo.conf.server_wide_modules = " "['web', 'anybox_homepage']\n" in gu
            )

    def test_install_scripts_10_gunicorn_proxied(self):
        self.make_recipe(
            version="local %s" % os.path.join(TEST_DIR, "odoo10"), gunicorn="proxied"
        )
        self.recipe.version_detected = "10.0alpha"
    ...
    def test_parse_odoo_scripts(self):
        ...
        self.assertEqual(
            ...
            dict(
                script_name=dict(entry="myentry", command_line_options=[]),
                withargs=dict(
                    entry="withargs", arguments="session", command_line_options=[]
                ),
                ...
                nosetests_odoo=dict(entry="nosetests", command_line_options=["-d"]),
            ),
        )
