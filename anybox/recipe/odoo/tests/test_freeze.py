...
class TestFreeze(RecipeTestCase):
    ...
    def test_freeze_egg_versions(self):
        ...
        try:
            ...
            self.fail(
                "Expected version of %s " "egg not dumped !" % self.fictive_dist_name
            )
    ...
    def test_freeze_egg_versions_develop(self):
        ...
        self.recipe._freeze_egg_versions(conf, "freeze")
        self.assertRaises(NoOptionError, conf.get, "freeze", self.fictive_dist_name)
    ...
    def test_freeze_vcs_source_already_frozen(self):
        ...
        subprocess.check_call(
            ["hg", "--cwd", repo_path, "tag", "sometag", "-u", COMMIT_USER_FULL, "-q"]
        )

        self.assertEqual(
            self.recipe._freeze_vcs_source("hg", repo_path, "sometag"), "sometag"
        )
        self.assertNotEqual(
            self.recipe._freeze_vcs_source("hg", repo_path, "default"), "default"
        )
    ...
    def test_freeze_vcs_source_dirty(self):
        ...
        subprocess.check_call(["hg", "-q", "--cwd", repo_path, "revert", "--all"])
    ...
    def test_prepare_frozen_buildout_gp_vcsdevelop(self):
        ...
        self.assertEquals(
            extends_develop.strip(),
            "fakevcs+http://example.com/aeroolib@fakerev" "#egg=aeroolib",
        )
