...
class TestExtraction(RecipeTestCase):
    ...
    def test_extract_addons(self):
        ...
        addons = ["local specific", "fakevcs http://some/repo vcs-addons revspec"]
        self.make_recipe(version="local mainsoftware", addons=os.linesep.join(addons))
        ...
        self.assertEquals(extracted, set([os.path.join(target_dir, "vcs-addons")]))
        ...
        self.failIf(
            os.path.exists(os.path.join(target_dir, "specific")),
        ...
        with open(os.path.join(target_dir, "vcs-addons", ".fake_archival.txt")) as f:
    ...
    def test_prepare_extracted_buildout_gp_vcsdevelop(self):
        ...
        self.assertEquals(
            set(d for d in develop if d), set(["aeroolib", "simple_develop"])
        )
    ...
    def test_prepare_extracted_buildout_gp_vcsdevelop_develop_dir(self):
        ...
        develop = conf.get("buildout", "develop").split(os.linesep)
        self.assertEquals(
            set(d for d in develop if d), set(["src/aeroolib", "simple_develop"])
        )
    ...
    def test_prepare_extracted_buildout_remove_bzr(self):
        ...
        self.assertEqual(
            conf.get(self.recipe.name, "recipe"), "anybox.recipe.odoo:server"
        )
    ...
    def test_prepare_extracted_buildout_keep_other_extras(self):
        ...
        self.recipe._extract_sources(conf, target_dir, set())
        self.assertEqual(
            conf.get(self.recipe.name, "recipe"), "anybox.recipe.odoo[other]:server"
        )
    ...
    def test_extract_downloads_to(self):
        ...
        self.assertEqual(
            ext_conf.get("odoo", "addons").splitlines(),
            ["local target", "local somwehere", "local stdl"],
        )

        self.assertEqual(ext_conf.get("odoo", "version").strip(), "local parts/odooo")
        self.assertEqual(
            ext_conf.get("versions", self.fictive_name), self.fictive_version
        )
