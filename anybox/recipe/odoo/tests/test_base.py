...
class TestBaseRecipe(RecipeTestCase):
    ...
    def test_base_url(self):
        self.make_recipe(version="10.0-1", base_url="http://example.org/odoo")
        self.assertDownloadUrl("http://example.org/odoo/blob-10.0-1.tgz")

    def test_base_url_nightly(self):
        self.make_recipe(
            version="nightly 10.0rc1c latest", base_url="http://example.org/odoo"
        )
        self.assertDownloadUrl("http://example.org/odoo/10-0-nightly-latest.tbz")
    ...
    def test_parse_addons_revisions(self):
        ...
        self.make_recipe(
            version="bzr lp:openobject-server server last:1" " option=option"
        )
        recipe = self.recipe

        recipe.parse_revisions(dict(revisions="1234"))
        self.assertEquals(
            recipe.sources[main_software],
            ("bzr", ("lp:openobject-server", "1234"), dict(option="option")),
        )
        ...
        self.assertEquals(
            recipe.sources["addons-specific"],
            ("hg", ("http://some/repo", "default"), {"opt": "spam"}),
        )
        ...
        # with a comment
        recipe.parse_revisions(
            dict(revisions="1112 ; main software" "\naddons-specific 1.0")
        )
    ...
    def test_parse_addons_illformed(self):
        ...
        for illformed in (
            ...
            self.assertRaises(UserError, recipe.parse_addons, dict(addons=illformed))
    ...
    def test_clean(self):
        ...
        self.make_recipe(
            ...
            addons=os.linesep.join(
                ("local addon-dir", "fakevcs http://some/repo vcs-addons revspec")
            ),
            clean="true",
        )
        ...
        for path, expected in (
            ...
            self.assertEquals(os.path.exists(os.path.join(b_dir, *path)), expected)

    def test_clean_vcs_server(self):
        """Test clean for base class vcs server."""
        self.make_recipe(
            version="fakevcs http://some/where server-dir somerev", clean="true"
        )
        ...
        for path, expected in (
            ...
            self.assertEquals(
                os.path.exists(os.path.join(server_path, *path)), expected
            )
    ...
    def test_vcs_revert(self):
        ...
        self.make_recipe(version="fakevcs http://some/where server-dir mainrev")
    ...
    def test_finalize_addons_paths_git_layout(self):
        self.make_recipe(version="git http://github.com/odoo/odoo.git odoo 10.0")
    ...
    def test_finalize_addons_paths_bzr_layout(self):
        ...
        self.recipe.finalize_addons_paths(check_existence=False)
        self.assertEquals(
            self.recipe.addons_paths, [base_addons, "/some/separate/addons"]
        )
    ...
    def test_finalize_addons_paths_order(self):
        ...
        os.makedirs(base_addons)
        os.makedirs(odoo_addons)
        self.recipe.addons_paths = ["/some/separate/addons", odoo_addons]
        self.recipe.finalize_addons_paths(check_existence=False)
        self.assertEquals(
            self.recipe.addons_paths,
    ...
    def make_recipe_appplying_requirements_file(self, reqs_content):
        ...
        with open(os.path.join(self.recipe.odoo_dir, "requirements.txt"), "w") as f:
    ...
    def test_apply_requirements_file(self):
        ...
        self.assertTrue(
            dist_name in self.recipe.requirements,
    ...
    def test_apply_requirements_file_precedence1(self):
        ...
        dist_name = "someproject"
        self.make_recipe_appplying_requirements_file("%s==1.2.3" % dist_name)
        versions = self.apply_requirements_file(pre_versions={dist_name: "17.2"})
    ...
    def test_list_develops(self):
        ...
        self.assertEqual(self.recipe.list_develops(), [self.fictive_dist_name])

    def test_apply_requirements_file_precedence2(self):
        """Unit test for Odoo requirements.txt: develops should win"""
        self.make_recipe_appplying_requirements_file(self.fictive_dist_name + "==6.2.3")
        self.develop_fictive()
        versions = self.apply_requirements_file()
