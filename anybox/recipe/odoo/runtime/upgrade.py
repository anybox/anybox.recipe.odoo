...
def upgrade(upgrade_script, upgrade_callable, conf, buildout_dir):
    ...
    * ``buildout_dir``: directory of the buildout
    ...
    if not quiet:
        print(
            "Starting upgrade, logging details to %s at level %s, "
    ...
    if session.is_initialization:
        logger.info("Database %r base initialization done. Proceeding further", db_name)
    else:
    ...
    if pkg_version is None:
        logger.warn(
            "Expected package version file %r does not exist. "
        ...
        logger.info(
            "Read package version: %s from %s", pkg_version, session.version_file_path
        )
    ...
    if db_version is None:
        if not session.is_initialization:
            logger.warn(
                "No version currently set in database (the present "
    ...
    upgrade_module = imp.load_source("anybox.recipe.odoo.upgrade_odoo", upgrade_script)
    ...
    if statuscode is None or statuscode == 0:
        ...
        session.close()
        logger.info(
            "%s successful. Total time: %d seconds."
            % (
