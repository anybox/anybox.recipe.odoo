...
class ServerRecipe(BaseRecipe):
    ...
    with_upgrade = True
    ws = None
    template_upgrade_script = os.path.join(os.path.dirname(__file__), "upgrade.py.tmpl")
    server_wide_modules = ()

    def __init__(self, *a, **kw):
        super(ServerRecipe, self).__init__(*a, **kw)
        opt = self.options
        self.with_devtools = opt.get("with_devtools", "false").lower() == "true"
    ...
    def apply_version_dependent_decisions(self):
        ...
        if gunicorn == "proxied":
            ...
            logger.warn(
                "'gunicorn = proxied' now superseded since "
    ...
    def _create_gunicorn_conf(self, qualified_name):
        ...
        gunicorn_options.update(
            (k[len(gunicorn_prefix) :], v)
            for k, v in self.options.items()
            if k.startswith(gunicorn_prefix)
        )

        gunicorn_options["server_wide_modules"] = (
            list(self.server_wide_modules) if self.server_wide_modules else ["web"]
        )
...
"""
        ...
        for opt, val in self.options.items():
            ...
            if opt == "log_level":
                ...
                val = dict(
                    ...
                    ERROR=40,
                    CRITICAL=50,
                ).get(val.strip().upper(), 30)
            if opt in SERVER_COMMA_LIST_OPTIONS:
                val = [i.strip() for i in val.split(",")]

            conf += "conf[%r] = %r" % (opt, val) + os.linesep

        preload_dbs = option_splitlines(self.options.get("gunicorn.preload_databases"))
        if preload_dbs:
    ...
    def _parse_odoo_scripts(self):
        ...
        for line in option_splitlines(self.options.get("odoo_scripts")):
            ...
            if not naming or len(naming) > 2:
                ...
                name = naming[1]
            cl_options = []
            desc = scripts[name] = dict(
                entry=naming[0], command_line_options=cl_options
            )
            ...
            for token in line[1:]:
                if token.startswith(opt_prefix):
                    ...
                    if level not in dir(logging):
                        raise UserError(
                            "In script %r, improper logging " "level %r" % (name, level)
                        )
                    desc["odoo_log_level"] = level
                else:
                    raise UserError("Invalid token for script %r: %r" % (name, token))
    ...
    def _register_main_startup_script(self, qualified_name):
        ...
        desc = self._get_or_create_script("odoo_starter", name=qualified_name)[1]
        ...
        if self.server_wide_modules:
            arguments += ", server_wide_modules=%r" % (self.server_wide_modules,)
    ...
    def _register_test_script(self, qualified_name):
        ...
        desc = self._get_or_create_script("odoo_tester", name=qualified_name)[1]
    ...
    def _register_upgrade_script(self, qualified_name):
        desc = self._get_or_create_script("odoo_upgrader", name=qualified_name)[1]
        script_opt = option_strip(self.options.get("upgrade_script", "upgrade.py run"))
        ...
        desc.update(
            entry="odoo_upgrader",
            arguments="%r, %r, %r, %r"
            % (script_source_path, script[1], self.config_path, self.buildout_dir),
        )

        if not os.path.exists(script_source_path):
            logger.warning(
                "Ugrade script source %s does not exist." "Initializing it for you",
                script_source_path,
            )
    ...
    def _register_gunicorn_startup_script(self, qualified_name):
        ...
        """
        desc = self._get_or_create_script("gunicorn", name=qualified_name)[1]
        ...
        if gunicorn_entry_point is None:
            gunicorn_entry_point = "odoo:" "service.wsgi_server.application"
    ...
    def _register_gevent_script(self, qualified_name):
        ...
        desc = self._get_or_create_script("odoo-gevent", name=qualified_name)[1]
    ...
    def _register_cron_worker_startup_script(self, qualified_name):
        ...
        if not os.path.isfile(script_src):
            ...
            if self.version_wanted == "6.1-1" or (
                version.startswith("6.1-2012") and version[4:12] < "20120530"
            ):
                logger.warn(
                    "Can't use odoo-cron-worker with version %s "
                    "You have to run a separate regular Odoo process "
                    "for cron jobs to be launched.",
                    version,
                )
                return

            logger.info(
                "Cron launcher odoo-cron-worker not found in "
            ...
            script_src = join(os.path.split(__file__)[0], "odoo-cron-worker")

        desc = self._get_or_create_script("odoo_cron_worker", name=qualified_name)[1]
        desc.update(
            entry="odoo_cron_worker",
            ...
            if log_level:
                initialization = os.linesep.join(
                    (
                        initialization,
                        "import logging",
                        "logging.getLogger('odoo').setLevel" "(logging.%s)" % log_level,
                    )
                )
    ...
    def _install_startup_scripts(self):
        ...
        self.eggs_reqs.extend(
            (
                ("odoo_starter", "anybox.recipe.odoo.runtime.start_odoo", "main"),
                ("odoo_cron_worker", "anybox.recipe.odoo.runtime.start_odooo", "main"),
                ("odoo_upgrader", "anybox.recipe.odoo.runtime.upgrade", "upgrade"),
            )
        )
        ...
        gevent_script_name = self.options.get(
            "gevent_script_name", "gevent_%s" % self.name
        )
