...
class CommonTestCase(testing.VcsTestCase):
    ...
    def create_src(self):
        ...
        subprocess.call(
            ["hg", "commit", "-m", "initial commit", "-u", testing.COMMIT_USER_FULL]
        )

    def test_str(self):
        repo = BaseRepo("/some/path", "http://some/url")
        self.assertEqual(
            str(repo), "BaseRepo at '/some/path' " "(remote='http://some/url')"
        )

    def test_unknown(self):
        self.assertRaises(UserError, get_update, "unknown", "", "", "default")
    ...
    def test_retry(self):
        ...
        # now the retry
        get_update(
            "hg_fails_updates", repo_path, self.src_repo, "default", clear_retry=True
        )
